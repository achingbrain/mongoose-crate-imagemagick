...
ImageMagick.prototype.createFieldSchema = function() {
	...
	Object.keys(this._options.transforms).forEach(function(name) {
		...
		output[name].depth = String;
		output[name].width = String;
		output[name].height = String;
	});
	...
	tasks.push(function(callback) {
		async.waterfall([function(callback) {
			...
			storageProvider.save(outputFile, function(error, url) {
				callback(error, outputFile, attributes, stats, url);
			});
		}], function(error, outputFile, attributes, stats, url) {
